 ...
 * Calculator.java 
 *
 * Java class for calculating sunset and sunrise times.  This class  exposes
 * two methods only: getSunsetTimeUTC and getSunriseTimeUTC.  Both take all the
 * parameters necessary to make the calculation; that is, nothing is assumed,
 * and no system information is used.  If the caller wants to make, for
 * example, the current year or geographical location the default, then it has
 * to do that itself. The caller also needs to supply the Sun's zenith, which
 * is subject to some interpretation. For most purposes, the constant ZENITH
 * defined in this class will do the trick for true sunrise and sunset. There
 * are other zenith constants for various types of twilight.
 *
 * The methods in this class return a UTC time, not a local time.  This allows
 * its operation to be independent of the system. If the caller requires a
 * local time, which is likely, it needs to convert it.  Methods are provided
 * in the Java <code>TimeZone</code> class to do this, but they rely on the
 * system time zone and clock to be correct.
 *
 * (c)2001-2022 Kevin Boone, GPL v3.0
 */
...
//  that corresponds to one hour time difference.
  ...
  /**
   GetSunriseTimeUTC_z gets the time of sunrise at the specified latitude and
   longitude on the specified day, with specified zenith. Result is returned in a
   Time object
   <pre>
   year	   4-digit year
   month      month, 1-12
   ...
   */
  public static Time getSunriseTimeUTC_z (int year, int month, int day,
  ...
  /**
   getSunsetTimeUTC_z gets the time of sunrise at the specified latitude and
   longitude on the specified day with the specified zenith. Result is returned in
   a Time object
   <pre>
   year	   4-digit year
   month      month, 1-12
   ...
   */
  public static Time getSunsetTimeUTC_z (int year, int month, int day,
      double longitude, double latitude, double zenith)
      throws SunTimesException
  {
  ...
  /**
   * As getSunsetTimeUTC_z but with default zenith
   */
  public static String getSunsetTimeUTC (int year, int month, int day,
      double longitude, double latitude)
      throws SunTimesException
  {
    return getSunsetTimeUTC_z (year, month, day, longitude, latitude,
        ZENITH).toString();
  }

  /**
   * As getSunriseTimeUTC_z but with default zenith
   */
  public static String getSunriseTimeUTC (int year, int month, int day,
      double longitude, double latitude)
      throws SunTimesException
  {
    return getSunriseTimeUTC_z (year, month, day, longitude, latitude,
        ZENITH).toString();
  }
